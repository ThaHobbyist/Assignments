...
def save_file(final_string, name):
    directory = os.path.dirname("parsable_strings/")
    if not os.path.exists(directory):
        os.makedirs(directory)

    with open("parsable_strings/{}.txt".format(name), 'w') as f:
...
if __name__ == '__main__':
    ...
    try:
        ...
        if accepted:
            ...
            save_file(parsing_table, compressed_name)

            print("The string {0} is parsable! \n".format(string))
        else:
